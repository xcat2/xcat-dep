...
int smVirtual_Network_Vswitch_Set(struct _VmApiInternalContext* vmapiContextP,
	...
	int inputSize = 4 + 4 + strlen(functionName) + 4 + strlen(userid) + 4
			...
			switchName) + 4 + strlen(grantUserid) + 4 + strlen(userVlanId) + 4
			+ strlen(revokeUserid) + 4 + strlen(realDeviceAddress) + 4
			+ strlen(portName) + 4 + strlen(controllerName) + 1
			/* connectionValue */+ 4 /* queueMemoryLimit */+ 1 /* routingValue */+ 1
			/* porttype */+ 1 /* updateSystemConfigIndicator */+ 4 + strlen(
