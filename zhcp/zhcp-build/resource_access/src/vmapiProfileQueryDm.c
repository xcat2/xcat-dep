...
int smProfile_Query_DM(struct _VmApiInternalContext* vmapiContextP,
	...
	if (cacheFileValid(vmapiContextP, cachePath)) {
		...
		if (cacheFileP) {
			...
			sprintf(traceLine,
					"Profile cache file(%s) pointer opened for read: %p \n",
					profileID, cacheFileP);
			...
			TRACE_START(vmapiContextP, TRACEAREA_CACHE, TRACELEVEL_DETAILS);
			sprintf(traceLine,
					"Profile cache file integer descriptor value: %d \n",
					cacheFileFD);
			TRACE_END_DEBUG(vmapiContextP, traceLine);
			...
			if (cacheFileFD != -1) {
				...
				if (fcntl(cacheFileFD, F_SETLK, &fl) != -1) {
					...
					TRACE_START(vmapiContextP, TRACEAREA_CACHE,
							TRACELEVEL_DETAILS);
					sprintf(traceLine,
							"Profile cache file read lock obtained.\n");
					TRACE_END_DEBUG(vmapiContextP, traceLine);
					...
					*outData = smMemoryGroupAlloc(vmapiContextP,
					...
					if (recordList == 0 || *outData == 0) {
						...
						errorLog(vmapiContextP, __func__, TO_STRING(__LINE__),
								RcRuntime, RsNoMemory, traceLine);
						TRACE_EXIT_FLOW(vmapiContextP, TRACEAREA_CACHE);
						return MEMORY_ERROR;
					}
					while (fgets(resultLine, sizeof(resultLine), cacheFileP)) {
						...
						recordList[recordCount - 1].profileRecordLength
								= profileRecordLen;
					...
					TRACE_START(vmapiContextP, TRACEAREA_CACHE,
							TRACELEVEL_DETAILS);
					sprintf(traceLine,
							"Profile cache file read lock to be unlocked.\n");
					TRACE_END_DEBUG(vmapiContextP, traceLine);
					...
					TRACE_START(vmapiContextP, TRACEAREA_CACHE,
							TRACELEVEL_DETAILS);
					sprintf(traceLine,
							"Profile cache file read lock unlocked.\n");
					TRACE_END_DEBUG(vmapiContextP, traceLine);
			...
			if (0 != fclose(cacheFileP)) {
				sprintf(traceLine,
						"***Error trying to close profile cache file.\n");
				errorLog(vmapiContextP, __func__, TO_STRING(__LINE__),
						RcFunction, RsUnexpected, traceLine);
				TRACE_EXIT_FLOW(vmapiContextP, TRACEAREA_CACHE);
				return PROCESSING_ERROR;
			...
			TRACE_START(vmapiContextP, TRACEAREA_CACHE, TRACELEVEL_DETAILS);
			sprintf(traceLine, "Profile cache file closed.\n");
			TRACE_END_DEBUG(vmapiContextP, traceLine);
	...
	// Cache file is invalid/missing/(out of date) remove it
		...
		if (remove(cachePath)) {
			if (ENOENT != errno) // If the error is anything but the file is not found
			{
				sprintf(
						traceLine,
						"***Error removing out of date profile cache file <%.*s> errno %d\n",
			...
			TRACE_START(vmapiContextP, TRACEAREA_CACHE, TRACELEVEL_DETAILS);
			sprintf(traceLine, "Profile cache file not found.\n");
			TRACE_END_DEBUG(vmapiContextP, traceLine);
		} else {
			TRACE_START(vmapiContextP, TRACEAREA_CACHE, TRACELEVEL_DETAILS);
			sprintf(traceLine, "Profile cache file erased, too old.\n");
			TRACE_END_DEBUG(vmapiContextP, traceLine);
		}
	}
		...
		if ((0 == (*outData)->common.returnCode) && (0
			...
			// Write the cache file
			{
				...
				if (NULL == cacheFileP) {
					sprintf(
							traceLine,
							"***Error trying to open profile cache file (%s) for write. errno %d\n",
							profileID, errno);
					errorLog(vmapiContextP, __func__, TO_STRING(__LINE__),
							RcFunction, RsUnexpected, traceLine);
					return PROCESSING_ERROR;
				...
				TRACE_START(vmapiContextP, TRACEAREA_CACHE, TRACELEVEL_DETAILS);
				sprintf(traceLine,
						"Profile cache file pointer opened for WRITE: %p \n",
						cacheFileP);
				TRACE_END_DEBUG(vmapiContextP, traceLine);
					...
					if (cacheFileFD != -1) {
						...
						if (fcntl(cacheFileFD, F_SETLK, &fl) != -1) {
							...
							TRACE_START(vmapiContextP, TRACEAREA_CACHE,
									TRACELEVEL_DETAILS);
							sprintf(traceLine,
									"Profile cache file WRITE lock obtained.\n");
							TRACE_END_DEBUG(vmapiContextP, traceLine);
							...
							if (tempSize > 0) {
								for (i = 0; i < tempSize; i++) {
									...
									memcpy(
											resultLine,
											(*outData)->profileRecordList[i].recordData,
									...
									if (EOF == rc) {
										...
										if (remove(cachePath)) {
											sprintf(
													traceLine,
													"***Error removing profil cache file after write error  <%.*s> errno %d\n",
													LINESIZE, cachePath, errno);
											errorLog(vmapiContextP, __func__,
													TO_STRING(__LINE__),
													RcFunction, RsUnexpected,
							...
							if (-1 == fcntl(cacheFileFD, F_SETLK, &fl)) {
								...
								sprintf(traceLine,
										"***Error trying to unlock profile cache file WRITE lock.\n");
								errorLog(vmapiContextP, __func__, TO_STRING(
							...
							TRACE_START(vmapiContextP, TRACEAREA_CACHE,
									TRACELEVEL_DETAILS);
							sprintf(traceLine,
									"Profile cache file write lock unlocked.\n");
							TRACE_END_DEBUG(vmapiContextP, traceLine);
