...
int smVirtual_Network_Vswitch_Create(
	...
	int inputSize = 4 + 4 + strlen(functionName) + 4 + strlen(userid) + 4
			...
			switchName) + 4 + strlen(realDeviceAddress) + 4 + strlen(portName)
			+ 4 + strlen(controllerName) + 1 /* connectionValue */+ 4
			/* queueMemoryLimit */+ 1 /* routingValue */+ 1 /* transportType */+ 4
			/* vlanId */+ 1 /* porttype */+ 1 /* updateSystemConfigIndicator */+ 4
			+ strlen(systemConfigName) + 4 + strlen(systemConfigType) + 4
			+ strlen(parmDiskOwner) + 4 + strlen(parmDiskNumber) + 4 + strlen(
			parmDiskPassword) + 4 + strlen(altSystemConfigName) + 4 + strlen(
			altSystemConfigType) + 4 + strlen(altParmDiskOwner) + 4 + strlen(
			altParmDiskNumber) + 4 + strlen(altParmDiskPassword) + 1
			/* gvrpValue */+ 4 /* nativeVlanId */;
	char * inputP = 0;
	char * smapiOutputP = 0;
	char line[LINESIZE];
	int i;
