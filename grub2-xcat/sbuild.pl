#!/usr/bin/env perl
# grub2-xcat/sbuild.pl -- per-package Ubuntu/Debian builder for grub2-xcat; the apt/sbuild analogue of
# grub2-xcat/mockbuild.pl. Builds the package inside the matching <codename>-<arch>-sbuild chroot from
...
# BuildUtils::build_deb_in_chroot; only grub2-xcat's build is below.
use strict;
use warnings;
use Cwd qw(abs_path);
...
# grub2-xcat is Architecture:all with the source laid out IN the package dir, so it is built in place
# (no tarball to extract). Built once on amd64 (see debs-manifest.conf); dpkg-buildpackage drops the
# .deb(s) in the parent of the source dir, which the collector picks up.
my $build = <<'BUILD';
set -e
