#!/usr/bin/env perl
# elilo/sbuild.pl -- per-package Ubuntu/Debian builder for elilo-xcat; the apt/sbuild analogue of
# elilo/mockbuild.pl. Builds the package inside the matching <codename>-<arch>-sbuild chroot from its
...
# BuildUtils::build_deb_in_chroot; only elilo's source prep + dpkg-buildpackage is below.
use strict;
use warnings;
use Cwd qw(abs_path);
...
# elilo-xcat is Architecture:all (x86/EFI loader) -> built once on amd64 (see debs-manifest.conf).
my $build = <<'BUILD';
set -e
